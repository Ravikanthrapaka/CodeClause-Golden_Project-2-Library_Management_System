...
def addbooks():
    ...
    con.commit()
    win.destroy()
    messagebox.showinfo("Book", "Book is Added Sucessfully")
    closedb()
    libr()
...
def issuebooks():
    ...
    con.commit()
    win.destroy()
    messagebox.showinfo("Book", "Book is Issued Sucessfully")
    closedb()
    libr()
def returnbook():
    global win
...
def admin():
    ...
    win.geometry("400x400+480+180")
    win.resizable(False,False)
    b1=Button(win, height=2,width=25,text=' Add New User ',command=adduser)
...
def adduser():
    ...
    mob=Label(win,text='CREATE PASSWORD')
    global e1,b
    e1=Entry(win,width=25)
    global e2
    e2=Entry(win,width=25)
...
def connectdb():
    global con,cur
    #Please Enter the  your username and password of MySQL.
...
def home():
    try:
        ...
        window=Tk()
        window.title('Welcome to Library')
